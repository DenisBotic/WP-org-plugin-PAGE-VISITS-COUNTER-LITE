...
*       Also listend for change in select-by-type and run the check.
*       Reveal the module so it can be trigered from the select/deselect all module.
		...
		*/
		function enableIconsMenu() {
				...
				if ( list_type_obj.open_list_type === "list-hidden" ) {
						// LIST HIDDEN
						// Enable all icon menu buttons except set-hidden button
						icon_menu_btns_hidden_list.removeClass('StrCPVisits_icon_btn_disabled');

				} else {
						// LIST VISIBLE
						// Enable all icon menu button except set-visible button
